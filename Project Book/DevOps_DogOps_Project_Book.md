...
אחד האלמנטים המתקדמים בפרויקט הוא השימוש ב-**ApplicationSets**. במקום ליצור ידנית הגדרה ב-ArgoCD עבור כל סביבה וכל אפליקציה, נכתב קובץ תבנית אחד שסורק את תיקיית ה-`environments` ומייצר דינמית את האפליקציות עבור Test, Staging ו-Prod. זהו יישום ברמת Enterprise שמאפשר צמיחה וניהול קלים (Scalability).
